...
const signup = async (req, res) => {
  try {
    ...
    if (booking.status !== "approved") {
      return res
        .status(400)
        .json({ message: "Your booking is not approved yet" });
    }
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ message: "Account already exists for this booking" });
    }
...
const assignHostel = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Hostel assigned successfully", user: updatedUser });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
