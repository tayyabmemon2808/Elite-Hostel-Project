...
const addHostel = async (req,res) => {
    try{
        const {name, city, address, description, images, singleRoomPrice, sharedRoomPrice} = req.body;
        const newHostel = new Hostel({
            name, city, address, description, images, singleRoomPrice, sharedRoomPrice
        })
...
const assignSubAdmin = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Sub-admin assigned successfully', hostel: updatedHostel });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
};

module.exports = { addHostel, getAllHostels, getHostelById, updateHostel, deleteHostel, assignSubAdmin };