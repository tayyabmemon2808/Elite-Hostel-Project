...
const createStayEntry = async (req, res) => {
  try {
    const { student, room, hostel } = req.body;

    const roomData = await Room.findById(room);
    const roommatesAtThatTime = roomData.studentsAllotted.filter(s => s.toString() !== student);

    const newEntry = new StayHistory({
      student,
      room,
      hostel,
    ...
    res.status(201).json({ message: 'Stay entry created', entry: newEntry });
  } catch (error) {
...
module.exports = { createStayEntry, checkoutStay, getMyStayHistory };